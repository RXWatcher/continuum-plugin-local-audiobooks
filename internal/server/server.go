...
function readConfigForm(){return {metadata_sources_enabled:document.getElementById("metadata-sources").value.split(",").map(x=>x.trim()).filter(Boolean),metadata_scan_source:document.getElementById("metadata-scan-source").value.trim(),metadata_default_region:document.getElementById("metadata-region").value.trim(),metadata_cache_ttl_days:Number(document.getElementById("metadata-cache-ttl").value)||30,metadata_rate_limit_rps:Number(document.getElementById("metadata-rate-limit").value)||5,scan_inline_enrich:document.getElementById("scan-inline").checked,standalone_http_listen:document.getElementById("standalone-listen").value.trim(),stream_signing_secret:document.getElementById("stream-secret").value.trim()}}
function writeConfigForm(c){document.getElementById("metadata-sources").value=(c.metadata_sources_enabled||[]).join(", ");document.getElementById("metadata-scan-source").value=c.metadata_scan_source||"audnexus";document.getElementById("metadata-region").value=c.metadata_default_region||"us";document.getElementById("metadata-cache-ttl").value=c.metadata_cache_ttl_days||30;document.getElementById("metadata-rate-limit").value=c.metadata_rate_limit_rps||5;document.getElementById("scan-inline").checked=Boolean(c.scan_inline_enrich);document.getElementById("standalone-listen").value=c.standalone_http_listen||"";document.getElementById("stream-secret").value=c.stream_signing_secret||""}
