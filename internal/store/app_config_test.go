...
func TestAppConfigDefaults(t *testing.T) {
	...
	cfg.MetadataScanSource = "audiobookcovers"
	if err := cfg.Validate(); err == nil {
		t.Fatal("Validate accepted a non-scan source")
	}
}
...
func TestAppConfigSeedAndLegacyImport(t *testing.T) {
	...
	if dsn == "" {
		t.Skip("LOCAL_AUDIOBOOKS_TEST_DSN unset; skipping integration app config test")
	}
	...
	legacy.StandaloneHTTPListen = ":7879"
	...
	if cfg.MetadataDefaultRegion != "gb" || cfg.StandaloneHTTPListen != ":7879" {
