...
type AppConfig struct {
	...
	StreamSigningSecret    string   `json:"stream_signing_secret"`
}
...
func (c AppConfig) Validate() error {
	...
	if !validScanSource(c.MetadataScanSource) {
		return fmt.Errorf("metadata_scan_source is not a valid scan-capable source: %s", c.MetadataScanSource)
	}
	return nil
}
...
func validMetadataSource(id string) bool {
	switch id {
	case "audnexus", "audimeta", "itunes", "storytel", "bookbeat", "audioteka", "audiobookcovers":
		return true
	default:
		return false
	}
...
func validScanSource(id string) bool {
	switch id {
	case "audnexus", "audimeta", "itunes", "storytel", "bookbeat", "audioteka":
		return true
	default:
		return false
	}
