// Command silo-plugin-local-audiobooks is the plugin entrypoint.
package main
...
import (
	...
	"github.com/RXWatcher/silo-plugin-local-audiobooks/internal/grpc/metadataprovider"
	"github.com/RXWatcher/silo-plugin-local-audiobooks/internal/httproutes"
	...
	"github.com/RXWatcher/silo-plugin-local-audiobooks/internal/migrate"
	...
	"github.com/RXWatcher/silo-plugin-local-audiobooks/internal/scheduler"
	"github.com/RXWatcher/silo-plugin-local-audiobooks/internal/server"
...
func main() {
	logger := hclog.New(&hclog.LoggerOptions{Name: "silo-plugin-local-audiobooks"})
	...
	configureMetadata := func(p *pgxpool.Pool, st *store.Store, appCfg store.AppConfig) {
		ua := "silo-local-audiobooks/" + manifest.GetVersion()
